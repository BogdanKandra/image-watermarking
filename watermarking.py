...
def visible_watermark2(wmImage, hostImage):
    """Embeds a watermark image all over the host image, using the visible 
    ...
    # Add the watermark image to the overlay, repeating it horizontally and
    # vertically as needed
    for x in range(repeatX):
        for y in range(repeatY):
            overlay[wmH * y:wmH * (y + 1), wmW * x: wmW * (x + 1)] = wmImage
    #overlay[imageH - wmH - 10:imageH - 10, imageW - wmW - 10:imageW - 10] = wmImage
